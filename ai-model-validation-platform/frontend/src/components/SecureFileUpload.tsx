...
import { IntervalHandle, TimeoutHandle, safeClearTimeout, safeClearInterval, safeSetTimeout, safeSetInterval } from '../utils/timerUtils';
import {
  Box,
  Button,
  Typography,
...
const SecureFileUpload: React.FC<SecureFileUploadProps> = ({
  ...
  const cleanupRef = useRef<IntervalHandle | null>(null);
  const uploadIntervalRefs = useRef<Map<string, IntervalHandle>>(new Map());
  const uploadTimeoutRefs = useRef<Map<string, TimeoutHandle>>(new Map());
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (cleanupRef.current) {
        safeClearInterval(cleanupRef.current);
      }
      
      // Clear all upload progress intervals
      const intervals = uploadIntervalRefs.current;
      intervals.forEach(interval => safeClearInterval(interval));
      intervals.clear();
      
      // Clear all upload completion timeouts
      const timeouts = uploadTimeoutRefs.current;
      timeouts.forEach(timeout => safeClearTimeout(timeout));
      timeouts.clear();
    };
  }, []);
  ...
  const getStatusColor = (file: UploadFile): import('../services/types').ChipColor => {
  ...
  // Memoize current upload files length to prevent stale closures
  const currentUploadFilesLength = useMemo(() => uploadFiles.length, [uploadFiles.length]);

  const startFileUpload = useCallback(async (uploadFile: UploadFile): Promise<void> => {
    if (!isMountedRef.current) return;
    ...
    try {
      ...
      uploadIntervalRefs.current.set(uploadFile.id, progressInterval);
      ...
      const completionTimeout = safeSetTimeout(() => {
        ...
        const interval = uploadIntervalRefs.current.get(uploadFile.id);
        if (interval) {
          safeClearInterval(interval);
          uploadIntervalRefs.current.delete(uploadFile.id);
        }
        
        // Remove timeout reference
        uploadTimeoutRefs.current.delete(uploadFile.id);
        ...
        setUploadFiles(prev => {
          ...
          // Call completion callback with the updated files
          const completedFiles = newFiles.filter(f => f.status === 'completed');
          if (completedFiles.length > 0 && isMountedRef.current) {
            ...
            safeSetTimeout(() => {
              if (isMountedRef.current) {
                try {
                  ...
                  // Error in upload completion callback: callbackError
      ...
      uploadTimeoutRefs.current.set(uploadFile.id, completionTimeout);

    } catch (error) {
      // Error in startFileUpload: error
      
      if (!isMountedRef.current) return;
      
      // Clean up any intervals/timeouts for this file
      const interval = uploadIntervalRefs.current.get(uploadFile.id);
      if (interval) {
        safeClearInterval(interval);
        uploadIntervalRefs.current.delete(uploadFile.id);
      ...
      const timeout = uploadTimeoutRefs.current.get(uploadFile.id);
      if (timeout) {
  ...
  const validateAndProcessFiles = useCallback(async (files: FileList | File[]) => {
    ...
    try {
      ...
      if (currentUploadFilesLength + fileArray.length > maxFiles) {
      ...
      // Validate each file with proper error handling
      const validationPromises = newUploadFiles.map(async (uploadFile) => {
        if (!isMountedRef.current) return;
        ...
        try {
          ...
          if (validationResult.errors.length === 0 && isMountedRef.current) {
            return startFileUpload(uploadFile);
          }
        } catch (error) {
          if (!isMountedRef.current) return;
      ...
      // Wait for all validations with proper error handling
      try {
        await Promise.allSettled(validationPromises);
      } catch (error) {
        // Error during file validation batch: error
        onUploadError('Some files failed to validate properly');
      }
      ...
      // Error in validateAndProcessFiles: error
      if (isMountedRef.current) {
        setIsValidating(false);
        onUploadError('Failed to process files');
      }
    }
  ...
  const handleCancel = useCallback((fileId: string) => {
    ...
    // Clean up any ongoing operations for this file
    const interval = uploadIntervalRefs.current.get(fileId);
    if (interval) {
      safeClearInterval(interval);
      uploadIntervalRefs.current.delete(fileId);
    }
    
    const timeout = uploadTimeoutRefs.current.get(fileId);
    if (timeout) {
      safeClearTimeout(timeout);
      uploadTimeoutRefs.current.delete(fileId);
    }
  ...
  const handleRetry = useCallback((fileId: string) => {
    ...
    if (file && file.status === 'failed') {
      ...
      // Use promise handling for retry
      startFileUpload(file).catch(_error => {
        // Retry upload failed: error
        if (isMountedRef.current) {
          onUploadError('Retry upload failed');
        }
      });
    }
  }, [uploadFiles, startFileUpload, onUploadError]);
