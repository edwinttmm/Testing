...
import { Project, TestSession, TestResult, VideoFile, VideoStatus } from '../services/types';
import { apiService, startEnhancedTestWorkflow, stopEnhancedTestWorkflow, getEnhancedTestWorkflowStatus, getEnhancedTestWorkflowResults, createEnhancedTestSession, runEnhancedTestSession, getEnhancedTestSessionResults, getAllVideos, getVideoDetections, validateVideo } from '../services/api';
...
import { RealDetectionPanel } from '../components/RealDetectionPanel';
...
import HardwareSignalPanel from '../components/HardwareSignalPanel';
...
const EnhancedTestExecution = () => {
  ...
  // HIL Test Mode state
  const [hilTestMode, setHilTestMode] = useState(false);
  const [currentVideoPlaylist, setCurrentVideoPlaylist] = useState<VideoFile[]>([]);
  ...
  const [timingMetrics, setTimingMetrics] = useState({ latency: 0, responseTime: 0, accuracy: 0 });
  ...
  return (
    <Box sx={{ 
                        ...
                        if (videos.length === 0) {
                          ...
                          const reliableTestVideos: VideoFile[] = [
                            ...
                            },
                            {
                              ...
                              uploadedAt: new Date().toISOString(),
                              status: VideoStatus.VALIDATED,
                              processingStatus: VideoStatus.VALIDATED,
                              groundTruthGenerated: false,
                              detectionCount: 0,
                              annotationCount: 0,
                              createdAt: new Date().toISOString()
            ...
            <FixedGrid xs={12} sm={6} md={4}>
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    await Promise.all(selectedVideos.map(v => validateVideo(v.id, true)));
                    showSnackbar('All videos validated successfully', 'success');
                  } catch (err) {
                    showSnackbar('Failed to validate videos', 'error');
                  }
                }}
      ...
      {(() => {
        ...
        return selectedVideos.length > 0 ? (
          ...
          config={{
            ...
            // Additional video playback options - commented out for compatibility
          ...
          onError={(error) => {
            ...
            const videoIndex = (safeError && typeof safeError === 'object' && 'videoIndex' in safeError) ? (safeError as any).videoIndex : 0;
            const message = (safeError && typeof safeError === 'object' && 'message' in safeError) ? (safeError as any).message : 'Unknown error';
            showSnackbar(
              `❌ Video ${videoIndex + 1} error: ${message}`,
              'error'
            );
          }}
          ...
          showControls={true}
          showProgress={true}
          // showQueue={true}  // Commented out - not supported by current interface
      ...
      {/* Real Backend Detection Panel - Uses actual AI detection data */}
      <RealDetectionPanel
        videos={selectedVideos}
        projectId={selectedProject?.id}
        toleranceMs={testConfig.latencyMs || 100}
        onDetectionComplete={(result) => {
          console.log('✅ Real detection data loaded:', result);
          if (result.success && result.total_detections > 0) {
            // Store real detection results for HIL testing
            console.log(`🎯 HIL Testing ready with ${result.total_detections} validated objects from AI Backend`);
          }
        }}
      />
