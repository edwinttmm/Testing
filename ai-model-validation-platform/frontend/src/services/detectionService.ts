  ...
  private convertDetectionsToAnnotations(
    ...
    return detections.map((det, index) => {
      ...
      if (Array.isArray(bboxArray) && bboxArray.length >= 4) {
        ...
        bbox = {
          x: safeGet(bboxArray, 'x', 0) as number,
          y: safeGet(bboxArray, 'y', 0) as number,
          width: safeGet(bboxArray, 'width', safeGet(bboxArray, 'w', 100)) as number,
          height: safeGet(bboxArray, 'height', safeGet(bboxArray, 'h', 100)) as number
        };
      } else {
        // Fallback to object properties directly on detection
        bbox = {
          x: safeGet(det, 'x', 0) as number,
          y: safeGet(det, 'y', 0) as number,
          width: safeGet(det, 'width', safeGet(det, 'w', 100)) as number,
          height: safeGet(det, 'height', safeGet(det, 'h', 100)) as number
        };
      }
