...
/**
 * Generate dynamic video URL from video ID using new backend API endpoint
 * This resolves the video file path dynamically from the database
 */
export function getDynamicVideoUrl(videoId: string): string {
  const config = getServiceConfig('api');
  return `${config.url}/api/videos/${videoId}/file`;
}

/**
 * Get video stream URL with proper headers for video playback
 */
export function getVideoStreamUrl(videoId: string): string {
  return getDynamicVideoUrl(videoId);
}

/**
 * Check if dynamic video URL is accessible
 */
export async function isDynamicVideoAccessible(videoId: string): Promise<boolean> {
  try {
    const response = await fetch(getDynamicVideoUrl(videoId), { method: 'HEAD' });
    return response.ok;
  } catch (error) {
    logger.warn(`Dynamic video ${videoId} not accessible:`, error);
    return false;
  }
}
