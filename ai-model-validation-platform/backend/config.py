...
# Import BaseSettings from pydantic-settings (required for Pydantic v2)
try:
    ...
    try:
        ...
        print("ERROR: Please install pydantic-settings: pip install pydantic-settings")
        raise
...
class Settings(BaseSettings):
    ...
    base_data_directory: str = os.getenv('AIVALIDATION_BASE_DATA_DIR', os.path.dirname(os.path.abspath(__file__)))
    screenshots_directory: str = os.getenv('AIVALIDATION_SCREENSHOTS_DIR', 'screenshots')
    enable_path_migration: bool = os.getenv('AIVALIDATION_ENABLE_PATH_MIGRATION', 'true').lower() == 'true'
    path_validation_strict: bool = os.getenv('AIVALIDATION_PATH_VALIDATION_STRICT', 'true').lower() == 'true'
