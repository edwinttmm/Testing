...
class TestCompleteWorkflow:
    ...
    async def test_end_to_end_ground_truth_generation(self, integration_tester, test_project):
        ...
        try:
            ...
            updated_video = db.query(Video).filter(Video.id == video_record.id).first()
...
class TestDataFlowValidation:
    ...
    def test_database_transaction_integrity(self, integration_tester, test_project):
        ...
        db = SessionLocal()
        initial_gt_count = db.query(GroundTruthObject).count()
        initial_detection_count = db.query(DetectionEvent).count()
        db.close()
        ...
        try:
            ...
            video = db.query(Video).filter(Video.id == video_record.id).first()
