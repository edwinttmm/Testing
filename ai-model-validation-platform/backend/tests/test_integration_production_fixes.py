...
from services.session_completion_service import complete_test_session
...
def test_complete_session_workflow_all_fixes_active(db_session, sample_session, sample_detections, sample_ground_truth):
    ...
    comparisons = db_session.query(DetectionComparison).filter_by(
        session_id=sample_session.id,
        match_type='TP'
    ).all()
...
def test_gt_double_matching_prevention(db_session, sample_session):
    ...
    tp_count = db_session.query(DetectionComparison).filter_by(
        session_id=sample_session.id,
        match_type='TP'
    ).count()
    fn_count = db_session.query(DetectionComparison).filter_by(
        session_id=sample_session.id,
        match_type='FN'
    ).count()
