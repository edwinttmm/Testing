...
Original location: tests/test_video_id_reassignment.py
"""
...
class TestVideoIDReassignmentEdgeCases:
    ...
    def test_detection_at_exact_video_start_boundary(self, service, mock_video_timing_map):
        ...
        assert assigned_video == "video-1-id", "Detection at exact boundary should belong to previous video"
    ...
    def test_detection_at_exact_video_end_boundary(self, service, mock_video_timing_map):
        ...
        # Boundary detections stay with the video that was actively playing
        assert assigned_video == "video-1-id", "Detection at boundary should remain with previous video"
    ...
    def test_overlapping_buffer_zones(self, service):
        ...
        # Stay with previous video while timing sync catches up
        assert assigned_video == "video-1-id", "Detection in overlap should remain with previous video"
    ...
    async def test_reassignment_with_mixed_null_and_valid_video_ids(self, service):
        ...
        with patch('services.detection_video_reassignment.SessionLocal') as mock_session_local:
            ...
            first_call_iter = iter([mock_session, mock_video_seq])

            def first_side_effect(*args, **kwargs):
                try:
                    return next(first_call_iter)
...
class TestVideoIDReassignmentIntegration:
    ...
    async def test_session_completion_triggers_reassignment(self):
        ...
        with patch('services.detection_video_reassignment.reassign_null_video_ids') as mock_reassign:
            ...
            with patch('services.session_completion_service.SessionLocal') as mock_session_local, \
                 patch('services.session_completion_service.validate_video_sequence_completion', return_value=(True, "")):
                mock_db = MagicMock()
                mock_session_local.return_value = mock_db
