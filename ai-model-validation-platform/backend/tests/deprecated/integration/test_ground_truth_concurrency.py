...
Original location: tests/integration/test_ground_truth_concurrency.py
"""
...
class TestConcurrentVideoProcessing:
    ...
    async def test_multiple_video_processing_async(self, concurrency_helper):
        ...
        try:
            for video in videos:
                updated_video = db.execute(select(Video).where(Video.id == video.id)).scalar_one_or_none()
...
class TestDatabaseConcurrency:
    ...
    def test_transaction_isolation(self, concurrency_helper):
        ...
        def transaction_test(thread_id):
            ...
            try:
                ...
                try:
                    ...
                    # Read initial count
                    initial_count = session.execute(select(func.count()).select_from(GroundTruthObject).where(
                        GroundTruthObject.video_id == video.id
                    )).scalar()
                    ...
                    mid_count = session.execute(select(func.count()).select_from(GroundTruthObject).where(
                        GroundTruthObject.video_id == video.id
                    )).scalar()
                    ...
                    # Final count
                    final_count = session.execute(select(func.count()).select_from(GroundTruthObject).where(
                        GroundTruthObject.video_id == video.id
                    )).scalar()
        ...
        db = SessionLocal()
        try:
            final_total = session.execute(select(func.count()).select_from(GroundTruthObject).where(
                GroundTruthObject.video_id == video.id
            )).scalar()
...
class TestRaceConditionPrevention:
    ...
    def test_duplicate_processing_prevention(self, concurrency_helper):
        ...
        db = SessionLocal()
        try:
            video_state = db.execute(select(Video).where(Video.id == video.id)).scalar_one_or_none()
    ...
    def test_concurrent_status_updates(self, concurrency_helper):
        ...
        def update_video_status(thread_id, new_status):
            ...
            try:
                db = SessionLocal()
                try:
                    video_record = db.execute(select(Video).where(Video.id == video.id)).scalar_one_or_none()
                    if video_record:
        ...
        db = SessionLocal()
        try:
            final_video = db.execute(select(Video).where(Video.id == video.id)).scalar_one_or_none()
            ...
            try:
                ...
                try:
                    ...
                    for video_id in video_ids:
                        ...
                        video = db.execute(select(Video).where(Video.id == video_id).with_for_update()).scalar_one_or_none()
