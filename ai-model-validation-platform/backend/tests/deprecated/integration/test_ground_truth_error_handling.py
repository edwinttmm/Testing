...
Original location: tests/integration/test_ground_truth_error_handling.py
"""
...
class TestDatabaseErrors:
    ...
    def test_transaction_rollback_on_error(self, error_helper):
        ...
        db = SessionLocal()
        initial_count = db.execute(select(func.count()).select_from(GroundTruthObject)).scalar()
        db.close()
        ...
        db = SessionLocal()
        try:
            final_count = db.execute(select(func.count()).select_from(GroundTruthObject)).scalar()
...
class TestSystemRecovery:
    ...
    def test_processing_retry_mechanism(self, error_helper):
        ...
        db = SessionLocal()
        try:
            failed_video = db.execute(select(Video).where(Video.id == video.id)).scalar_one_or_none()
        ...
        db = SessionLocal()
        try:
            recovered_video = db.execute(select(Video).where(Video.id == video.id)).scalar_one_or_none()
